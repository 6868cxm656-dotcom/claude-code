...
| Deploys from | branch **`main`** of the PRIVATE repo `tcf-mission-control`, root `/`, `npx wrangler deploy` | push = deploy, ~60s. Root `wrangler.jsonc` is the config the build uses |
...
| Budget data pipeline | `budget-dashboard/` (this repo) | `build.py` turns the budget workbook + Xero variance report into `dashboard.html`; import that file in-app. Never commit the outputs |
...
| Offline backup copy | `offline/tcf-risk-register.html` (this repo) | read-only viewer; copy it from db-app/public/index.html in the same commit (see §5) |
| Plans & history | `db-app/MISSION_CONTROL_PLAN.md` | the plan docs record *why*, commit messages record *what* |
| The OLD public repo | `6868cxm656-dotcom/claude-code` (a public fork) | retired July 2026. Its git history — including the June 2026 risk baseline — is public forever; never push app code there again |
...
- **Hidden budget panels never leave the server** for non-admins — same
  ...
  *independent* per-panel payloads (`buildBudgetPanels` in logic.mjs); the
  ...
  decoration.
- **Budget figures live ONLY in D1.** This repo is private, but keep the
  discipline anyway: never commit a built dashboard, a budget workbook, or
  real monthly figures. The module ships empty and is fed by the in-app
  admin import. (This invariant exists because the app once lived in a
  public repo — see §6.)
...
5. git push origin main                 # that IS the deploy
6. hard-refresh, check footer version, click the thing you changed
7. sync the offline copy (same repo, same commit is fine):
   cp db-app/public/index.html offline/tcf-risk-register.html
```
...
- **The app used to live in a PUBLIC fork** (`6868cxm656-dotcom/claude-code`).
  It was migrated here in July 2026 because the seed baseline — including the
  hidden risk R41 and SLT emails — was world-readable there, and GitHub forks
  cannot be made private. Treat everything in that repo's history as public:
  the June 2026 baseline is out; the live register (everything edited since)
  never left the database. Do not push app code to the old repo again.

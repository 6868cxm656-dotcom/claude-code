...
- **Committee pack**: a printable PDF snapshot per committee (Board sees everything) — overview KPIs, the committee's register, detail boxes for flagged/outside-appetite risks, and a change log since that committee's last recorded meeting date. Meeting dates are stored as full schedules per committee; the pack auto-selects the last past meeting and shows the next one.
- **QA approval workflow**: changes by risk owners (edits, new risks, deletions) do **not** go live — they are held as proposals. Jim and Julia (admins) approve or reject each one on the **Approvals** tab, with a full before/after diff; approved changes apply with the audit entry attributed to the proposer and marked "approved by". Admin edits apply immediately (they are the QA). Owners are prompted for a note and a pre-addressed email to the admins opens on submission. Proposals travel with exports and are reconciled on merge, so an owner's proposal made in their browser reaches the admins' copy via the normal export/import.
